...
const util = {
  ...
  getExtensionFileVscodeResource: function (context, relativePath) {
    ...
    console.log(
      ...
      relativePath,
      diskPath,
      diskPath.with({ scheme: 'vscode-resource' }).toString()
    );
  ...
  readFileList(dir, filesList = []) {
    ...
    files.forEach((item, index) => {
      ...
      if (
        ...
        util.readFileList(path.join(dir, item), filesList); //递归读取文件
      } else {
  ...
  // 计算相对路径 参数1是目标路径 参数2是文本本身路径
  relativeDir(relative, absolute) {
